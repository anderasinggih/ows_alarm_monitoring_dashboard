<template>
	<div class="modal fade" id="ttCorrelationModalOlt" aria-hidden="true" aria-labelledby="modalFilterNetdrone" tabindex="-1">
		<div class="modal-dialog modal-xxxl modal-dialog-centered">
			<div class="modal-content p-3">
				<div class="d-flex flex-row justify-content-between align-items-center">
					<div class="box-row">
						<h4 class="font-700 fit-content">TT Correlation</h4>
						<div class="d-flex flex-row justify-content-center" v-if="this.oltDown.state.ttCorrelation.ttTableLoading == true">
				...
				<div class="modal-body">
					<div class="row d-flex flex-row mb-3" v-if="this.oltDown.store.ttCorrelation.data.length != 0">
						<div class="col-show-3 pe-1 ps-1" v-for="(data, index) in this.oltDown.store.ttCorrelation.data" :key="index">
							<CardTicketCounting :title="data.severity" :mtte="Math.abs(data.mtte)" :mttr="data.mttr" :closed="data.closed_count" :running="data.running_count" :cardColor="oltDown.store.ttCorrelation.cardColors[index]"></CardTicketCounting>
						</div>
					</div>
					...
					<div class="row d-flex flex-row mb-2 box-row" style="overflow-x: scroll">
						<table class="table table-striped table-bordered table-hover font-14" ref="ttTable">
							...
							<tbody v-if="this.oltDown.store.ttCorrelation.tableData != null || this.oltDown.store.ttCorrelation.tableData != undefined">
...
import { getCountTtCorrelationDetailsOlt } from "../../views/Api/counting/getCountTtCorrelationDetailsOlt.js";
...
export default {
	...
	watch: {
		state(newVal, oldVal) {
			if (newVal > oldVal) {
				this.oltDown.state.ttCorrelation.modalState = newVal;
				this.getTableData();
			}
		}
	},
	computed: {
		filteredData() {
			return [...this.oltDown.store.ttCorrelation.tableData].sort((a, b) => {
	...
	methods: {
		async getTableData() {
			this.oltDown.state.ttCorrelation.ttTableLoading = true;
