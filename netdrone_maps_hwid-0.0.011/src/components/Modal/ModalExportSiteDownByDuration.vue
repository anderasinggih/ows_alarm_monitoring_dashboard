<template>
	<div class="modal fade" id="exportSiteDownDuration" aria-hidden="true" aria-labelledby="modalFilterNetdrone" tabindex="-1">
			...
			<div class="modal-content p-3">
				<div class="d-flex flex-row justify-content-between align-items-center">
					<div class="box-row gap-8">
						<h4 class="font-700 fit-content">Export - {{ this.siteDown.state.modalExport.title }}</h4>
						<div v-if="this.siteDown.state.modalExport.loading == true" class="bg-card-loader"></div>
					</div>
					<div class="box-row">
						<a v-if="siteDown.state.modalExport.isMaximize == false" @click="maximizeModal()">
							<i class="bi bi-arrows-fullscreen font-24 text-grey"></i>
						</a>

						<a v-if="siteDown.state.modalExport.isMaximize == true" @click="minimizeModal()">
				...
				<div class="modal-body">
					<div class="row d-flex justify-content-end mb-3">
						<div class="col-6 gap">
							<div class="input-group width-35">
								...
								<input type="text" class="form-control search-global-input" placeholder="Search by Site ID" v-model="this.siteDown.state.modalExport.searchQuery" />
							</div>
						</div>
						...
						<table class="table table-striped table-bordered table-hover font-14" ref="exportSiteDownDetailTable" style="white-space: nowrap; height: fit-content">
							<thead>
								<tr>
									...
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">Initial RCA</th>
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">ACC</th>
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">ACC Site</th>
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">ACC Genset</th>
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">AGG</th>
									...
									<th class="align-content-center text-center font-inter font-700" style="color: #313131; font-size: 14px">Progress</th>
							...
							<tbody>
								<tr v-for="(data, index) in paginatedData" :key="index">
									<td class="align-content-center text-left font-inter font-400">{{ (siteDown.state.modalExport.currentPage - 1) * siteDown.state.modalExport.rowsPerPage + (index + 1) }}</td>
									...
									<td class="align-content-center text-left font-inter font-400">{{ data.initial_rca != null ? data.initial_rca : "" }}</td>
									<td class="align-content-center text-left font-inter font-400">{{ data.acc_name != null ? data.acc_name : "" }}</td>
									<td class="align-content-center text-left font-inter font-400">{{ data.acc_site != null ? data.acc_site : "" }}</td>
									<td class="align-content-center text-left font-inter font-400">{{ data.genset_number != null ? data.genset_number : "" }}</td>
									<td class="align-content-center text-left font-inter font-400">{{ data.agg_name != null ? data.agg_name : "" }}</td>
									...
									<td class="align-content-center text-left font-inter font-400 progress-item" v-if="data.progress_label != 'merged'" :title="data.progress != null ? data.progress : ''">{{ data.progress != null ? data.progress : "" }}</td>
									<td class="align-content-center text-left font-inter font-400 progress-item" v-else :title="data.progress != null ? data.progress : ''" @click="openTt(data.parent_order_id)" style="cursor: pointer; color: #007bff">
									...
									<td class="align-content-center text-left font-inter font-400" @click="openTt(data.tt)" style="cursor: pointer; color: #007bff">{{ data.tt != null ? data.tt : "" }}</td>
									<td class="align-content-center text-left font-inter font-400">
										<span v-for="(woItem, index) in data.wo ? data.wo.split(';') : []" :key="index" @click="openWo(woItem)" style="cursor: pointer; color: #007bff"> {{ woItem }}{{ index == data.wo.split(";").length - 1 ? "" : ";" }} </span>
									</td>
					...
					<div class="pagination-container">
						<select class="form-select form-select-sm" aria-label="Rows per page" v-model="siteDown.state.modalExport.rowsPerPage" style="width: 150px !important">
							<option value="10">10 per page</option>
							<option value="20">20 per page</option>
							<option value="50">50 per page</option>
						</select>
						<button @click="this.previousPage()" :disabled="siteDown.state.modalExport.currentPage === 1" class="btn btn-export-modal" style="width: fit-content">Previous</button>
						<span class="page-number">{{ siteDown.state.modalExport.currentPage }} of {{ this.totalPages }}</span>
						<button @click="this.nextPage()" :disabled="siteDown.state.modalExport.currentPage === this.totalPages" class="btn btn-export-modal" style="width: fit-content">Next</button>
					</div>
				</div>
			</div>
		</div>
...
export default {
	name: "ExportSiteDownDuration",
	data() {
		return IndexData;
	},
	watch: {
		"siteDown.state.modalExport.step": function (newVal) {
			this.siteDown.state.modalExport.step = newVal;
		},
		"siteDown.state.modalExport.showing": function (newVal, oldVal) {
			if (newVal > oldVal) {
				this.siteDown.state.modalExport.currentPage = 1;
				this.siteDown.state.modalExport.loading == true;
				U.initCsrfToken();
				this.siteDown.state.modalExport.showing = newVal;
				this.getData();
			}
		}
	},
	computed: {
		isDurationStep() {
			const step = this.siteDown.state.modalExport.step;
			return step === "duration-range1" || step === "duration-range2" || step === "duration-range3" || step === "duration-range4" || step === "duration-range5";
		},
		filteredTableData() {
			// STEP
			var allSelectedData = this.siteDown.state.modalExport.data;
			var step = this.siteDown.state.modalExport.step;
			//API PARAMS
			var durationModel = this.siteDown.state.modalExport.durationModel;
			let searchQuery = this.siteDown.state.modalExport.searchQuery?.toLowerCase() || "";
		...
		paginatedData() {
			let pageRows = parseFloat(this.siteDown.state.modalExport.rowsPerPage);
			const start = (this.siteDown.state.modalExport.currentPage - 1) * pageRows;
	...
	methods: {
		maximizeModal() {
			this.siteDown.state.modalExport.isMaximize = true;
		...
		minimizeModal() {
			this.siteDown.state.modalExport.isMaximize = false;
			const modal = document.getElementById("modal-export-site-down-duration");
			const rowShowing = document.getElementById("row-show-table-data-site-down-export");
			if (modal && rowShowing) {
		...
		async getData() {
			this.siteDown.state.modalExport.data = [];
			// await getAllSiteDownDataTable(this.siteDown.apiParams.powerAlarm, this.siteDown.apiParams.hubType, [], [], "", "", this.siteDown.apiParams.vipCheck, this.siteDown.apiParams.siteType);
		...
		async exportTableData() {
			...
			XLSX.writeFile(wb, "NE_" + this.siteDown.state.modalExport.title + "_Summary_Export.xlsx");
		},
		async exportAllData() {
			...
			XLSX.writeFile(wb, "NE_" + this.siteDown.state.modalExport.title + "_Full_Export.xlsx");
		},
		...
		async openWo(orderid) {
			if (orderid) {
				let dataObj = {
					is_archived: false,
					app_name: "CorrectiveMaintenance",
					module_name: "CorrectiveMaintenance",
					process_name: "ID_75_1518168058873",
					order_id: orderid
				};
				MessageProcessor.process({
					url: "/adc-service/web/rest/v1/services/OrderQuery/OrderQuery/oq_order_page_query",
					...
					success: (result) => {
						...
						result.parameters.process_name = "ID_75_1518168058873";
						result.parameters.order_status = result.parameters.processstatus;
						if (result.isLegacy === "legacy") {
							delete result.parameters.taskid;
							const url = this.addParamsByObject("https://30f9-sg.teleows.com/adc-web/bpm/order-process/order-submit-panel.html", result.parameters);
		...
		previousPage() {
			if (this.siteDown.state.modalExport.currentPage > 1) {
				this.siteDown.state.modalExport.currentPage--;
			}
		},
		nextPage() {
			if (this.siteDown.state.modalExport.currentPage < this.totalPages) {
				this.siteDown.state.modalExport.currentPage++;
			}
		},
