<template>
	<div class="tab-pane fade" id="tt-list-site-pane" role="tabpanel" aria-labelledby="tt-list-site-tab" tabindex="0">
		<div class="row mb-3">
			<div class="col-6">
					...
					<div class="dropdown">
						...
						<ul class="dropdown-menu">
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTtTaskAll" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTtTaskCm" />
										<label class="form-check-label" for="flexCheckDefault"> Has CM </label>
									</div>
								</a>
							</li>
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTtTaskNonCm" />
										<label class="form-check-label" for="flexCheckDefault"> Not has CM </label>
									</div>
								</a>
							</li>
						</ul>
			...
			<div class="col-6">
				<div class="d-flex flex-row gap-12 justify-content-end">
					<div class="input-group width-250">
						...
						<input type="text" class="form-control search-global-input" id="searchGlobalInput" aria-describedby="searchGlobal" aria-label="Upload" placeholder="Search Task" v-model="this.globalProcess.store.site.query.siteTtSearchQuery" />
					</div>
				</div>
			</div>
		</div>
		<div class="row row-gap-16 row-task-list justify-content-unset">
			...
			<!-- FOR EACH COL-4 -->
			<div class="col-4" v-else v-for="data in filteredData" :key="data.ticket_id">
				<div class="accordion accordion-tasklist-border" :id="'taskListAccordion' + data.ticket_id">
					<div class="accordion-item">
						<h2 class="accordion-header py-1">
							<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" :data-bs-target="'#taskListAcr' + data.ticket_id" aria-expanded="false" :aria-controls="'taskListAcr' + data.ticket_id">
								<div class="row">
									<div class="col-12">
										<div v-if="data.task_type != undefined" class="row px-2" style="gap: 10px">
											<p class="task-types-class mb-2">{{ data.task_type }}</p>
										</div>
										<div v-if="data.task_type == undefined" class="row px-2" style="gap: 10px">
											<p class="task-types-class mb-2">{{ "-" }}</p>
										</div>
										<div class="row justify-content-between">
											<p class="font-18 width-fit-content">{{ data.ticket_id }}</p>
											<i class="bi bi-copy text-ioh font-18 width-fit-content" @click="copyTicketIdSite(data.ticket_id)"></i>
										</div>
									</div>
								</div>
							</button>
						</h2>
						<div :id="'taskListAcr' + data.ticket_id" class="accordion-collapse collapse" :data-bs-parent="'#taskListAccordion' + data.ticket_id">
							<div class="accordion-body">
								<div v-if="data.wo_id != undefined" class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">WO ID</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-500 text-dark">
											{{ data.wo_id }}
											<i v-if="data.wo_id" class="ms-3 bi bi-copy text-ioh font-16 c-pointer" @click="copyWoIdSite(data.wo_id)"></i>
										</p>
									</div>
								</div>
								...
								<!-- <div v-if="data.wo_id == undefined" class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">Alarm Name</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-600 text-dark">
											{{ data.alarm_name }}
										</p>
									</div>
								</div>
								...
								<div v-if="data.wo_id == undefined" class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">Last Occured Time</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-600 text-dark">
											{{ data.faultlastoccurtime }}
										</p>
									</div>
								</div> -->
								...
								<!-- <div v-if="data.wo_id != undefined" class="row mb-3">
									<div class="col-4">
								...
								<!-- <div v-if="data.wo_id != undefined" class="row mb-2">
									<div class="col-12">
										<button class="btn btn-apply width-100" @click="openEtaModal(data.wo_id)" data-bs-dismiss="modal">ETA Details3</button>
									</div>
								</div> -->
							</div>
						</div>
...
export default {
	name: "TabContentSiteTtList",
	data() {
		return IndexData;
	},
	computed: {
		filteredData() {
			const searchTerm = this.globalProcess.store.site.query.siteTtSearchQuery ? this.globalProcess.store.site.query.siteTtSearchQuery.toLowerCase().trim() : "";

			if (!this.globalProcess.store.site.ticket.siteTtList) {
				return [];
			}

			return this.globalProcess.store.site.ticket.siteTtList.filter((item) => {
				const matchesTaskType = this.globalProcess.store.site.checker.siteTtTaskAll || (this.globalProcess.store.site.checker.siteTtTaskCm && item.task_type === "CM") || (this.globalProcess.store.site.checker.siteTtTaskNonCm && item.task_type !== "CM") || item.task_type === undefined;
	...
	methods: {
		...
		async openEtaModal(woId) {
			const ticketDetails = this.globalProcess.store.site.ticket.siteTtList.find((item) => item.wo_id === woId);
