<template>
	<div class="tab-pane fade" id="task-list-pg-site-pane" role="tabpanel" aria-labelledby="task-list-pg-site-tab" tabindex="0">
		<div class="row mb-3">
			<div class="col-6">
					...
					<div class="dropdown">
						...
						<ul class="dropdown-menu">
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.sitePgTaskAll" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.sitePgTaskCm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.sitePgTaskPm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.sitePgTaskPlm" />
			...
			<div class="col-6">
				<div class="d-flex flex-row gap-12 justify-content-end">
					<div class="input-group width-250">
						...
						<input type="text" class="form-control search-global-input" id="searchGlobalInput" aria-describedby="searchGlobal" aria-label="Upload" placeholder="Search Task" v-model="this.globalProcess.store.site.query.sitePgSearchQuery" />
					</div>
				</div>
			</div>
		</div>
					...
					<div class="accordion-item">
						...
						<div :id="'taskListAcr' + data.wo_id" class="accordion-collapse collapse" :data-bs-parent="'#taskListAccordion' + data.wo_id">
							<div class="accordion-body">
								...
								<div class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">Suspect Cause2</p>
									</div>
									<div class="col-8">
...
export default {
	name: "TabContentTaskListPg",
	data() {
		return IndexData;
	},
	computed: {
		filteredData() {
			const searchTerm = this.globalProcess.store.site.query.sitePgSearchQuery ? this.globalProcess.store.site.query.sitePgSearchQuery.toLowerCase().trim() : "";

			if (!this.globalProcess.store.site.ticket.sitePgTicketList) {
				return [];
			}

			return this.globalProcess.store.site.ticket.sitePgTicketList.filter((item) => {
				const matchesTaskType =
					this.globalProcess.store.site.checker.sitePgTaskAll ||
					(this.globalProcess.store.site.checker.sitePgTaskCm && item.task_type === "CM") ||
					(this.globalProcess.store.site.checker.sitePgTaskPm && item.task_type === "PM") ||
					(this.globalProcess.store.site.checker.sitePgTaskPlm && item.task_type === "PLM");
	...
	methods: {
		...
		async openEtaModal(woId) {
			...
			const ticketDetails = this.globalProcess.store.site.ticket.sitePgTicketList.find((item) => item.wo_id === woId);
			...
			// if (ticketDetails) {
			// 	this.globalProcess.store.eta.pgTicketDetail = ticketDetails;
			// 	this.globalProcess.store.eta.pgOriginData = { lat: ticketDetails.fme_lat, lng: ticketDetails.fme_lon };
			// 	this.globalProcess.store.eta.pgDestinationData = { lat: ticketDetails.site_lat, lng: ticketDetails.site_lon };
			// }
			// const modalElement = document.getElementById("pgTrackingDetails");
			// if (modalElement) {
			// 	const pgTrackingDetails = new bootstrap.Modal(modalElement, {
			// 		backdrop: "static"
			// 	});
			// 	pgTrackingDetails.show();
			// } else {
			// 	console.error("Modal element with ID 'pgTrackingDetails' not found.");
			// }
			// etaTaskDetailPgTrackingMapsInit(this.globalProcess.store.eta.pgOriginData, this.globalProcess.store.eta.pgDestinationData);
		},
