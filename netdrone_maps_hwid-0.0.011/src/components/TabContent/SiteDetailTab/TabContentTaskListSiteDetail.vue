<template>
	<div class="tab-pane fade" id="task-list-site-pane" role="tabpanel" aria-labelledby="task-list-site-tab" tabindex="0">
		<div class="row mb-3">
			<div class="col-6">
					...
					<div class="dropdown">
						...
						<ul class="dropdown-menu">
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTaskAll" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTaskCm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTaskPm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="this.globalProcess.store.site.checker.siteTaskPlm" />
			...
			<div class="col-6">
				<div class="d-flex flex-row gap-12 justify-content-end">
					<div class="input-group width-250">
						...
						<input type="text" class="form-control search-global-input" id="searchGlobalInput" aria-describedby="searchGlobal" aria-label="Upload" placeholder="Search Task" v-model="this.globalProcess.store.site.query.siteSearchQuery" />
					</div>
				</div>
			</div>
		</div>
						...
						<div :id="'taskListAcr' + data.wo_id" class="accordion-collapse collapse" :data-bs-parent="'#taskListAccordion' + data.wo_id">
							<div class="accordion-body">
								...
								<div class="row mb-2">
									...
									<div class="col-8">
										<p class="font-12 font-500 text-dark">
											{{ !isNaN(Math.abs(parseFloat(data.sla_remain)))? Math.abs(parseFloat(data.sla_remain)) : data.sla_remain }}
										</p>
									</div>
								</div>
								...
								<div class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">FME ID</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-500 text-dark">{{ data.user_id }}</p>
									</div>
								</div>
								<div class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">FME Name</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-500 text-dark">{{ data.full_name }}</p>
									</div>
								</div>
								<div class="row mb-2">
									<div class="col-4">
										<p class="font-12 font-500 text-grey">FME Phone</p>
									</div>
									<div class="col-8">
										<p class="font-12 font-500 text-dark">{{ data.phone }}</p>
									</div>
...
export default {
	name: "TabContentTaskListSiteDetail",
	data() {
		return IndexData;
	},
	computed: {
		filteredData() {
			const searchTerm = this.globalProcess.store.site.query.siteSearchQuery ? this.globalProcess.store.site.query.siteSearchQuery.toLowerCase().trim() : "";

			if (!this.globalProcess.store.site.ticket.siteTicketList) {
				return [];
			}

			return this.globalProcess.store.site.ticket.siteTicketList.filter((item) => {
				const matchesTaskType =
					this.globalProcess.store.site.checker.siteTaskAll ||
					(this.globalProcess.store.site.checker.siteTaskCm && item.task_type === "CM") ||
					(this.globalProcess.store.site.checker.siteTaskPm && item.task_type === "PM") ||
					(this.globalProcess.store.site.checker.siteTaskPlm && item.task_type === "PLM");
	...
	methods: {
		...
		async openEtaModal(woId) {
			const ticketDetails = this.globalProcess.store.site.ticket.siteTicketList.find((item) => item.wo_id === woId);
