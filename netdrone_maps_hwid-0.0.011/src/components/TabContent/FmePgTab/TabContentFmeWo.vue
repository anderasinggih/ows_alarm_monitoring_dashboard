<template>
					...
					<div class="dropdown">
						...
						<ul class="dropdown-menu">
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="pg.store.fme.fmePgAll" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="pg.store.fme.fmePgCm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="pg.store.fme.fmePgPm" />
							...
							<li>
								<a class="dropdown-item">
									<div class="form-check">
										<input class="form-check-input" type="checkbox" value="" v-model="pg.store.fme.fmePgPlm" />
	...
	<div class="row row-ticket-list mb-3 row-gap-12 justify-content-unset">
		<div class="col-12" v-if="fmePgFilteredWoData == null || fmePgFilteredWoData == undefined || fmePgFilteredWoData.length == 0">
			<p class="text-center font-600 font-18 mt-4 mb-4">{{ this.pg.store.fme.fmePgWoMessage }}</p>
		</div>
		<div class="col-4" v-else v-for="data in fmePgFilteredWoData" :key="data.wo_id">
							...
							<div class="row">
								<div class="col-12">
									...
									<div class="row justify-content-between">
										...
										<i class="bi bi-copy text-ioh font-18 width-fit-content c-pointer" @click="copyWoIdFmePg(data.wo_id)"></i>
									</div>
								</div>
							</div>
						</button>
					...
					<div :id="'woFme' + data.wo_id" class="accordion-collapse collapse" :data-bs-parent="'#woFme' + data.wo_id">
						<div class="accordion-body">
							<div class="row mb-2">
								...
								<div class="col-8">
									<a data-bs-dismiss="modal" class="font-16 font-600 text-ioh text-decor-underline c-pointer" @click="selectSiteByIdPg(data.site_id_name, this.siteDown.apiParams.powerAlarm, data.site_lat, data.site_lon)">
										{{ data.site_id_name }}
									</a>
								</div>
							</div>
							...
							<div v-if="data.task_type == 'CM'" class="row mb-2">
								...
								<div class="col-8">
									<p class="font-16 font-500 text-dark me-2">
										{{ data.ticket_id }}
										<i v-if="data.ticket_id" class="ms-3 bi bi-copy text-ioh font-16 c-pointer" @click="copyTicketIdFmePg(data.ticket_id)"></i>
									</p>
								</div>
							</div>
...
export default {
	name: "TabContentFmeWo",
	data() {
		return IndexData;
	},
	watch: {
		"pg.store.fme.woList": function (newVal) {
			this.pg.store.fme.woList = newVal;
		},
	...
	computed: {
		fmePgFilteredWoData() {
			...
			if (!this.pg.store.fme.woList) {
				return [];
			}

			return this.pg.store.fme.woList.filter((item) => {
				const matchesTaskType = this.pg.store.fme.fmePgAll || (this.pg.store.fme.fmepgCm && item.task_type === "CM") || (this.pg.store.fme.fmePgPm && item.task_type === "PM") || (this.pg.store.fme.fmePgPlm && item.task_type === "PLM");
	...
	methods: {
		...
		async openEtaModal(woId) {
			...
			const ticketDetails = this.pg.store.fme.woList.find((item) => item.wo_id === woId);
		...
		async selectSiteByIdPg(siteid, power, lat, lon) {
			if (lat !== "" && lon !== "") {
