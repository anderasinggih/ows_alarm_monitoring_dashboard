		...
		<ul class="dropdown-menu p-2 dropdown-menu-end" style="min-width: 200px; overflow-y: hidden">
			...
			<div class="scroll-box">
				<li v-for="item in filteredItems" :key="item.area_name">
					<a class="dropdown-item" @click="selectItem(item.area_name, item.lat, item.lon, item.zoom)">{{ item.area_name }}</a>
				</li>
			</div>
		</ul>
...
export default {
	...
	methods: {
		...
		selectItem(item, lat, lon, zoom) {
			this.selectedItem = item;
			this.selectedLat = lat;
			this.selectedLon = lon;
			this.selectedZoom = zoom;
			this.$emit("itemSelected", item, lat, lon, zoom);
		},
		clearSelection() {
